...
class MelLoss(torch.nn.Module):
    ...
    def forward(self, estimated_spec, reference_spec):       
        mel_error_spec = torch.matmul(            
            (estimated_spec.to(self.device) - reference_spec.to(self.device)).abs()**2,
...
def main():
    writer = SummaryWriter(log_dir="/home/aac/shared/teams/amd-internal/huggingface/datasets/megatron-data/sopiko/speech_data/tensorboard/crn-mel")
